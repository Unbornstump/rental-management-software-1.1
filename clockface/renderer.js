...
// Initialize auth state from localStorage for browser environment
function initializeAuthFromStorage() {
  if (typeof localStorage !== 'undefined') {
    const token = localStorage.getItem('rms.authToken');
    const role = localStorage.getItem('rms.userRole');
    const username = localStorage.getItem('rms.username');
    const mustChangePassword = localStorage.getItem('rms.mustChangePassword') === 'true';
    ...
    // Start workspace loading if we have a token
    if (token) {
...
// Initialize auth from localStorage on page load (for browser environment)
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeAuthFromStorage);
} else {
